...
COPY --from=builder /go/src/pangoo/app-katalyst app-katalyst
COPY --from=builder /go/src/pangoo/conf conf
...
#docker run -w /data --name katalyst -d -p 20000:20000 -v /data/pangoo/conf:/data/conf  -v  /data/katalyst:/data/katalyst katalyst ./app-katalyst
