...
func Init(dsn string) (db *gorm.DB, err error) {
	...
	sqlDB, err = gorm.Open(mysql.Open(dsn),
		&gorm.Config {
			...
			DisableNestedTransaction: false,//默认开启嵌套事务false
	...
	sql_Db.SetMaxOpenConns(150)//连接池最多同时打开的连接数，这个maxOpenConns理应要设置得比mysql服务器的max_connections值要小
	sql_Db.SetMaxIdleConns(50)//连接池里最大空闲连接数，比SetMaxOpenConns小
	...
	sql_Db.SetConnMaxLifetime(1 * time.Hour)//连接池里面的连接最大存活时长

	data, _ := sonic.Marshal(sql_Db.Stats()) //获得当前的SQL配置情况
	hlog.Debug("DB conf: ", string(data))
	...
	//db.AutoMigrate()
	//根据model建表就放在各自模块了
	sqlDB.Set("gorm:table_options", "ENGINE=InnoDB")//.AutoMigrate()
	if err = sqlDB.Use(gormopentracing.New()); err != nil {
		hlog.Error(err)
		return nil, err
	}
	return sqlDB,nil
	...
	/*
	更新一个字段为”“或nil时，不能直接更新，需要通过take、save
